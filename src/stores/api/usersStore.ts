...
export const useUserStore = defineStore("userStore", () => {
  ...
  const createUser = async (form: UserForm) => {
    try {
      const param = await axios.post<CreateUserResponse>(
        "api/admin/users",
        form,
        {
          headers: { accept: "application/json" },
  ...
  const updateUser = async (form: UserUpdateForm, id: number) => {
    try {
      ...
      const errorMessage =
        error.response?.data?.message || "Error de red, intenta más tarde.";
